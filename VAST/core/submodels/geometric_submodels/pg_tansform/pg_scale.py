...
class ScaleToPrandtlGlauert(csdl.CustomExplicitOperation):
    """
    ...
    def define(self):
        ...
        rotational = self.parameters["rotational"]
        ...
        self.declare_derivatives("*", "Mach_number", method="cs")
        self.set_check_partial_options(wrt="Mach_number", method="fd", step=1e-8)

        row_col = np.arange(num_eval_points * 3)

        self.declare_derivatives("coll_pts_pg", "coll_pts_w_frame", rows=row_col, cols=row_col)
        self.declare_derivatives("force_pts_pg", "force_pts_w_frame", rows=row_col, cols=row_col)
        self.declare_derivatives("bound_vecs_pg", "bound_vecs_w_frame", rows=row_col, cols=row_col)
        ...
        for surface in surfaces:
            ...
            self.declare_derivatives(of_name, wrt_name, rows=row_col, cols=row_col)
    ...
    def compute_partials(self, inputs, partials):
        ...
        derivatives["bound_vecs_pg", "bound_vecs_w_frame"] = np.tile(fact, num_eval_pts)
        derivatives["coll_pts_pg", "coll_pts_w_frame"] = np.tile(fact, num_eval_pts)
        derivatives["force_pts_pg", "force_pts_w_frame"] = np.tile(fact, num_eval_pts)

        if rotational:
            fact_rot = np.array([betaPG**2, betaPG, betaPG], M.dtype)
            derivatives["rotational_velocities_pg", "rotational_velocities_w_frame"] = np.tile(fact_rot, num_eval_pts)
        ...
        for surface in self.parameters["surfaces"]:
            ...
            of_name = "{}_def_mesh_pg".format(name)
            nn = nx * ny
            derivatives[of_name, wrt_name] = np.tile(fact, nn)

            wrt_name = "{}_normals_w_frame".format(name)
            of_name = "{}_normals_pg".format(name)
            nn = (nx - 1) * (ny - 1)
            derivatives[of_name, wrt_name] = np.tile(fact_norm, nn)


class ScaleFromPrandtlGlauert(csdl.CustomExplicitOperation):
    """
