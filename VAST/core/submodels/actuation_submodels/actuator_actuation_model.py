...
class ActuatorActuationModel(csdl.Model):
    """
    ...
    def define(self):
        ...
        print('s_1_ind',s_1_ind)
        print('s_2_ind',s_2_ind)
        if s_2_ind==None:
        ...
        for i in range(num_surface):
            ...
            # h(x) = b (1 - ((x-a)/a)^2 )^0.5
            height = np.einsum('ik,j->ijk',np.ones((num_nodes,ny)),np.ones(x.shape)*0.21).reshape(num_nodes,nx,ny,1) #+ 0.03
