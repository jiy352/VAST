...
class EelActuationModel(csdl.Model):
    ...
    def define(self):
        ...
        for i in range(num_surface):
            ...
            area = np.sum(np.diff(x)*(b*(1-((x-a)/a)**2)**0.5)[:-1]

            plt.plot(x,b*(1-((x-a)/a)**2)**0.5)
            ...
            y = tail_amplitude_exp*((x_exp+linear_relation_exp)/(linear_relation_exp+1)) * csdl.sin(np.pi*2*x_exp/wave_number_exp - omg_exp*t_exp) * 0
