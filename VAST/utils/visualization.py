...
def run_visualization(surface_names, sim, h_stepsize,folder_name='',filename=''):
...
def run_visualization_eel(sim, h_stepsize):
    ...
    for i in range(len(surface_names)):
        ...
        for phase in np.linspace(0, 2 * np.pi, nframe + 1)[:nframe]:
            ...
            grid_mesh.save('eel_vtks/mesh'+str(i)+'.vtk')
            grid_bdmesh.save('eel_vtks/bd'+str(i)+'.vtk')
            grid.save('eel_vtks/wake'+str(i)+'.vtk')
            i+=1
