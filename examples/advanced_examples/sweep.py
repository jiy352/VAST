...
h_stepsize = t_vec[1]
v_x_list = [0.35,0.4,0.45]
tail_frequency_list = [0.48,0.7]
...
for i in range(2):
    ...
    np.savetxt('thrust_coeff_'+str(i)+'.txt',thrust_coeff)
    np.savetxt('thrust_'+str(i)+'.txt',thrust)
    np.savetxt('C_F'+str(i)+'.txt',sim['C_F'])
    np.savetxt('v_x_'+str(i)+'.txt',sim['v_x'])
