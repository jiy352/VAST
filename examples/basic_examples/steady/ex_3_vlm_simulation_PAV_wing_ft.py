...
theta = np.deg2rad(np.array([16, ]))  # pitch angles
...
surface_shapes = [(num_nodes, nx, ny*2, 3),]
...
r = R.from_euler('y', np.rad2deg(theta)+6.32979, degrees=True).as_matrix() # num_nodes,3,3
...
tail[:,:,0] = tail[:,:,0].copy() + 9.7
tail[:,:,2] = tail[:,:,2].copy() -0.85793/ft_t_meter
tail = np.einsum('ijk,lmk->ilmj', r, tail)

# exit()

wing = model_1.create_input('wing', val=rotated_mesh*ft_t_meter)
# wing = model_1.create_input('tail', val=np.einsum('i,jkl->ijkl', np.ones((num_nodes)), tail.reshape(nx_tail,ny_tail*2,3))*ft_t_meter)
