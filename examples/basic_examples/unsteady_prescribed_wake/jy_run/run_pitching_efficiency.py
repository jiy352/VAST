...
from VAST.core.submodels.output_submodels.vlm_post_processing.efficiency_new import EfficiencyModel
from VAST.utils.visualization import run_visualization
import os

########################################
# This is a test case to use simple cases
# to make a wing with the same AR as the fish 
# and run a pitching motion
########################################
...
def run_pitching_theodorsen_verification(k,num_nodes,N_period,A=1,save_results=False, save_vtk=False, path = "verfication_data/theodorsen"):
    ...
    chord = 1; 
    # span = 0.0638*2   # chord and span of the wing   
    span = 4   # chord and span of the wing   
    b = chord/2
    ...
    if save_vtk:
        run_visualization(surface_names=['wing'],sim=sim,folder_name='theodorsen_verfi',h_stepsize=h_stepsize)
    # del sim
    return sim,alpha
...
if __name__ == '__main__':
    ...
    efficiency_list = []    
    for k in k_list:
        sim, alpha = run_pitching_theodorsen_verification(k,num_nodes,N_period,A=A,save_vtk=True)
        CD_i_list.append(sim['wing_C_D_i'][int(num_nodes/N_period):])
        efficiency_list.append(sim['efficiency'])
        
    plt.legend(['k = '+str(i) for i in k_list])
        # plt.plot(alpha[int(num_nodes/N_period):], sim['wing_C_L'][int(num_nodes/N_period):])

    # sim = run_pitching_theodorsen_verification(k=0.2,num_nodes=num_nodes,N_period=N_period,A=A,save_vtk=False)
    # sim = run_pitching_theodorsen_verification(k=0.6,num_nodes=num_nodes,N_period=N_period,A=A,save_vtk=False)
    # sim, alpha = run_pitching_theodorsen_verification(k=1, num_nodes=num_nodes,N_period=N_period,A=A,save_vtk=False)
    # sim, alpha = run_pitching_theodorsen_verification(k=0.2, num_nodes=num_nodes,N_period=N_period,A=A,save_vtk=False)
    # sim, alpha = run_pitching_theodorsen_verification(k=1, num_nodes=num_nodes,N_period=N_period,A=A,save_vtk=False)
    # sim, alpha = run_pitching_theodorsen_verification(k=3, num_nodes=num_nodes,N_period=N_period,A=A,save_vtk=False)
    # sim, alpha = run_pitching_theodorsen_verification(k=5, num_nodes=num_nodes,N_period=N_period,A=A,save_vtk=False)
    ...
    # plt.legend(['VAST 1', 'theodorsen 1', ])
    # plt.xlabel(r'$\alpha$')
    # plt.ylabel(r'$C_l$')
    ...
    for i in range(len(k_list)):
        plt.plot(alpha[int(num_nodes/N_period):], CD_i_list[i],'.-')
        
    plt.legend(['k = '+str(i) for i in k_list])
    plt.xlabel(r'$\alpha$')
    plt.ylabel(r'$C_Di$')
    plt.savefig('verfication_data/theodorsen/C_D_i_AR4_k1.png',dpi=400,transparent=True)
    # plt.show()
    plt.figure()
    plt.plot(k_list, efficiency_list,'.-')
    plt.xlabel('k')
