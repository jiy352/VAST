...
def anderson_plunge(St,h_0_star):
    num_nodes = 150
    N_period=3
    alpha_0_deg = 15
    ...
    states_dict = {
        ...
        'theta': np.ones((num_nodes,1))*np.deg2rad(-5), 'psi': np.zeros((num_nodes, 1)),
    ...
    Pitching = PitchingModel(surface_names=['wing'], surface_shapes=[(nx,ny)], num_nodes=num_nodes,A=A*0, k=k,
...
sim_list=[]
for St in St_list:
    sim,t_vec, CT, CP = anderson_plunge(St,h_0_star)
    CT_list.append(CT)
    CP_list.append(CP)
